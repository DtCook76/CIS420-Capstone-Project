defmodule WorkbenchWeb.LocationLive.FormComponent do
  ...
  def update(%{location: location} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Garage.change_location(location))
     end)}
  end

  @impl true
  def handle_event("validate", %{"location" => location_params}, socket) do
    changeset = Garage.change_location(socket.assigns.location, location_params)
  ...
  defp save_location(socket, :edit, location_params) do
    case Garage.update_location(socket.assigns.location, location_params) do
      {:ok, location} ->
  ...
  defp save_location(socket, :new, location_params) do
    location_params = add_user_param(socket, location_params)
    case Garage.create_location(location_params) do
      {:ok, location} ->
        notify_parent({:saved, location})
